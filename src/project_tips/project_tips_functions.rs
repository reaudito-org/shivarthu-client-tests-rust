...
pub struct ProjectTipsStruct {
    driver: WebDriver,
}

impl ProjectTipsStruct {
    ...
    pub async fn new(driver: WebDriver) -> Result<Self, WebDriverError> {
        ...
        Ok(ProjectTipsStruct { driver })
    }

    pub async fn create_post(&self) -> WebDriverResult<()> {
        self.driver
            .goto(format!("{}/project-tips/create-project/{}", WEBPAGE_URL, 1))
            .await?;
        ...
        let tipper = self.driver.find(By::Id("tipperselect")).await?;
        tipper
            .find(By::XPath("./option[@value='SmallTipper']"))
            .await?
            .click()
            .await?;

        let fund_needed = self.driver.find(By::Id("funding-needed")).await?;

        fund_needed.send_keys("5000").await?;
    ...
    pub async fn apply_staking_period(&self) -> WebDriverResult<()> {
        ...
        self.driver
            .goto(format!(
                "{}/project-tips/apply-staking-period/{}",
                WEBPAGE_URL, 1
            ))
            .await?;
    ...
    pub async fn add_profile_stake(&self, stake: String, account_cut: &str) -> WebDriverResult<()> {
        ...
        let address = accounts_info["account1"]["ss58_address"].as_str().unwrap();
        self.driver
            .goto(format!("{}/add-profile-stake/{}", WEBPAGE_URL, 1))
            .await?;
    ...
    pub async fn challenge_evidence(&self, account_cut: &str) -> WebDriverResult<()> {
        ...
        let address = accounts_info["account1"]["ss58_address"].as_str().unwrap();
        self.driver
            .goto(format!("{}/project-tips/schelling-game/{}", WEBPAGE_URL, 1))
            .await?;
    ...
    pub async fn juror_selected_check(&self, account_to_check: &str) -> WebDriverResult<()> {
        ...
        let address = accounts_info["account1"]["ss58_address"].as_str().unwrap();
        self.driver
            .goto(format!("{}/project-tips-juror-selected/{}", WEBPAGE_URL, 1))
            .await?;
